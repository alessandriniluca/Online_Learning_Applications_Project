...
env = Environment(
    ...
    graph_clicks=np.array(
        [[.3, .2, .3, .5, .45], [.4, .3, .5, .6, .1], [.1, .8, .7, .2, .3], [.4, .7, .8, .3, .2], [.1, .8, .6, .8, .2]])
)
...
for single_budget in range(0, total_budget + resolution, resolution):
    for product_index in range(len(products)):
        for i, users in enumerate(users_number):
            ...
            alphas_prime[int(single_budget / resolution)][product_index][i] = (alphas[i] + delta_alpha_weights)[
                                                                                  product_index] / sum(
                (alphas[i] + delta_alpha_weights))
