...
class Optimizer:
    ...
    def __init__(self,
        ...
        self.number_of_campaigns = len(prices) * len(users_number) if exploit_context else len(prices)
    ...
    def compute_rows_with_context(self):
    ...
    def compute_rows_no_context(self):
        # for each user class
                ...
                for starting_prod in range(len(self.prices)):
                    ...
                    for arrival_prod in range(len(self.prices)):
                        ...
                        # else consider different quantity means of different classes
                            ...
                            expected_income_per_user += expected_income_per_product * self.mean_quantities[class_index][
                                arrival_prod]
    ...
    def run_optimization(self):
        self.reset()
        if self.exploit_context:
            self.compute_rows_with_context()
        else:
            self.compute_rows_no_context()
        self.build_final_table()

    def set_exploit_context(self, exploit_context):
        self.exploit_context = exploit_context
        self.reset()

    def reset(self):
        self.number_of_campaigns = \
            (len(self.prices) * len(self.users_number)) if self.exploit_context else len(self.prices)
        self.rows_income_per_budget = np.zeros((self.number_of_campaigns, self.number_of_budgets_to_evaluate))
        self.final_table = np.zeros((self.number_of_campaigns + 1, self.number_of_budgets_to_evaluate))
        self.partition = []
