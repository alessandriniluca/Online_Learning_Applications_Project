...
class Optimizer2:
                ...
                for starting_prod in range(len(self.mean_prices)):
                    ...
                    for arrival_prod in range(len(self.mean_prices)):
                        if self.mean_quantities.shape[0] == 1:
                            income += self.buy_prob[i][starting_prod][arrival_prod] * self.mean_prices[arrival_prod] * self.mean_quantities[0][arrival_prod]
                        else:    
                            income += self.buy_prob[i][starting_prod][arrival_prod] * self.mean_prices[arrival_prod] * self.mean_quantities[i][arrival_prod]
                    ...
                    if self.alphas.shape[2] == 1:
                        self.rows_income_per_budget[starting_prod][j] += int(self.alphas[j][starting_prod][0] * number) * income
                    else:
                        self.rows_income_per_budget[starting_prod][j] += int(self.alphas[j][starting_prod][i] * number) * income
    ...
    def find_best_allocation(self):
        ...
        budget_values = np.linspace(int(self.min_budget[0]),int(self.max_budget[0]), num=int(self.max_budget[0]/self.resolution)+1)
